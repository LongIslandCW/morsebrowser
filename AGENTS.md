...
- Automated tests: **Vitest** for unit/integration (utils, timing, lesson plugin, theme); **Playwright** for browser smoke E2E (accordions, pickers, Play, dark mode, accessibility). Full Web Audio / Knockout playback is not a first-pass Vitest target. Run `npm test` before PRs; E2E needs `npm run build` first (or `npm run test:all`). Accessibility copy/label changes should run `npx playwright test e2e/accessibility.spec.ts`. See `tests/README.md`. **OverLearn rich links** catalog (`tests/unit/lessons/overlearnRichLinks.test.ts`, `docs/OVERLEARN_RICH_LINKS.md`) is `describe.skip`'d while Tom iterates POL/display renames — re-enable when his preset work settles.
