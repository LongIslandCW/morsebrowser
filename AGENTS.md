...
- Do not auto-expand settings accordions (e.g. Voice Options) when Speed Racer or **Speak** is toggled; Roger rolled that back — user enables Voice and opens Voice Options manually.
...
- Voice Options: sub-controls stay visible and are **disabled** when Voice is off (not hidden). INT3 **Default** presets with `voiceRecap` (Arm Recap) disable the Voice master toggle while it still looks checked (`voiceEnabled` true + `manualVoice` from `voiceRecap`).
- **Speed Racer** (Jay; Lesson Options > Timing): multiplier-based per-card speed variations; mutually exclusive with Speed Intervals; no Keep FWPM UI (`speedRacerKeepFwpm` preset key kept for compat only). FWPM: Farnsworth when variation WPM exceeds saved FWPM; when variation WPM is slower, spacing scales down (`min(savedFwpm, variationWpm)`). **Replay Base Speed** and **Speak** / **Speak Before Replay** (label follows replay) in Advanced — Speak always enabled with Speed Racer; replay off → speak after last variation; pre-speak pad before recap. `speakSpeedRacerRecap` uses Voice Options when Speak is on (user turns Voice on manually). **Voice First** (`speakFirst`) is normal-playback only — bypassed during Speed Racer. Preset keys: `speedRacerEnabled`, `speedRacerMultipliers`, `speedRacerFinalPlay`, `speedRacerSpeakBeforeReplay`, `speedRacerKeepFwpm` (Overlearn = multipliers `1.0, 1.174, 1.348` + replay/speak off). Four **Speed Racer Overlearn** presets (`POL_*_SR.json`; display names swap leading **Overlearn** for **Speed Racer**) set **`voiceEnabled` false** and explicit **`speedRacerSpeakBeforeReplay` false** (legacymixin injects speak true if key missing). Repeat wordspaces in `cardBufferManager` go between repeats only (no trailing gap). Legacy defaults via `legacymixin.json`. When enabled, skip voice trail and Trail; exclude from `playEnded` voice/trail delays. Mobile CSS: `speed-racer-*`; E2E in `e2e/settings-layout-mobile.spec.ts`. **Doc:** [docs/SPEED_RACER.md](docs/SPEED_RACER.md).
