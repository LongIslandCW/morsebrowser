...
- **Speed Racer** (Jay; Lesson Options > Timing): multiplier-based per-card speed variations; mutually exclusive with Speed Intervals; Farnsworth forced off while racing; `speakSpeedRacerRecap` voice recap before optional base-speed replay. When enabled, skip voice trail (`addToVoiceBuffer`) and exclude from `playEnded` voice/trail delays (`needToSpeak` / `needToTrail`). Mobile layout uses `speed-racer-input-group` / `speed-racer-sequence-preview` / `speed-racer-multipliers-input` CSS; E2E overflow check in `e2e/settings-layout-mobile.spec.ts`.
