...
- Settings UI (`src/template.html`): **LICW Lessons** (expanded; TYPE/CLASS/CONTENT/LESSON/PRESETS as dropdown pickers), then five collapsed settings accordions — **Lesson Options**, **Voice Options**, **Tone & spacing Settings**, **Custom Input**, **Output Settings**. Trail, Speed Intervals, and Repeats live under Lesson Options; **Group & override** (Custom Group, Override time/size) sit in Lesson Options beside Practice. **Custom Input** holds View text, Clear, Insert File, and the practice textarea (moved out of the play toolbar).
