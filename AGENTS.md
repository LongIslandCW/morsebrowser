...
- **Speed Racer** (Jay; Lesson Options > Timing): multiplier-based per-card speed variations; mutually exclusive with Speed Intervals; **FWPM stays constant** during racing (only WPM scales per multiplier — Kurt's model); `speakSpeedRacerRecap` when **Speak Before Replay** is on (independent of Voice Options); **Replay Base Speed** and **Speak Before Replay** are separate toggles. Preset keys: `speedRacerEnabled`, `speedRacerMultipliers`, `speedRacerFinalPlay`, `speedRacerSpeakBeforeReplay` (Overlearn = multipliers `1.0, 1.174, 1.348` + both replays off). Legacy presets get Speed Racer defaults via `src/presets/legacymixin/legacymixin.json`. When enabled, skip voice trail (`addToVoiceBuffer`) and exclude from `playEnded` voice/trail delays (`needToSpeak` / `needToTrail`). Mobile layout uses `speed-racer-input-group` / `speed-racer-sequence-preview` / `speed-racer-multipliers-input` CSS; E2E overflow check in `e2e/settings-layout-mobile.spec.ts`.
